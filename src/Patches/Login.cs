...
public class Login : Il2CppSystem.Object
{
        ...
        ref Il2CppSystem.Collections.Generic.List<JSONNode> nodeList)
    {
        ...
        try {
            var url = LetheMain.ConfigServer.Value + "/custom/upload/" + dataClass;
            ...
            var schema = new ObjectPublicStHHDCStDeNDBoBPDaOFUnique(url, body.ToString(2), new Action<string>(_ => { }), "", false);
            Request.EnqueueWebRequest(HttpApiRequester.prop_HttpApiRequester_0, schema, true);
        }
        catch (Exception ex)
        {
